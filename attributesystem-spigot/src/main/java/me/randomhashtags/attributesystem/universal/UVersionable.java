package me.randomhashtags.attributesystem.universal;

import com.sun.istack.internal.NotNull;
import me.randomhashtags.attributesystem.AttributeSystemSpigot;
import me.randomhashtags.attributesystem.util.Versionable;
import org.bukkit.*;
...
public interface UVersionable extends Versionable {
    File DATA_FOLDER = AttributeSystemSpigot.getPlugin.getDataFolder();
    String SEPARATOR = File.separator;

    AttributeSystemSpigot EVENT_ATTRIBUTE_SYSTEM = AttributeSystemSpigot.getPlugin;
    FileConfiguration EVENT_ATTRIBUTE_SYSTEM_CONFIG = EVENT_ATTRIBUTE_SYSTEM.getConfig();
    String RP_VERSION = EVENT_ATTRIBUTE_SYSTEM.getDescription().getVersion();
    ...
    default void save(String folder, String file) {
        ...
        if(!f.exists()) {
            ...
            EVENT_ATTRIBUTE_SYSTEM.saveResource(hasFolder ? folder + SEPARATOR + file : file, false);
        }
    }
    ...
    default void sendStringListMessage(CommandSender sender, List<String> message, HashMap<String, String> replacements) {
        if(message != null && message.size() > 0 && !message.get(0).equals("")) {
            final boolean papi = EVENT_ATTRIBUTE_SYSTEM.placeholderapi, isPlayer = sender instanceof Player;

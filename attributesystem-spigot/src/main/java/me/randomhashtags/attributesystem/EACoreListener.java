...
public class EACoreListener extends EventExecutor implements Listener {
    ...
    public void unload() {
        EVENT_ATTRIBUTE_LISTENERS.clear();
        updateVersionListeners(false);
    }

    private void initVersionListeners() {
        if(EIGHT) {
            return;
        }
    ...
    private void updateVersionListeners(boolean register) {
        ...
        for(Listener listener : listeners) {
            if(register) {
                PLUGIN_MANAGER.registerEvents(listener, EVENT_ATTRIBUTE_SYSTEM);
            } else {
    ...
    public final void callEventAttributeListeners(@NotNull Event event) {
        for(EventAttributeListener listener : EVENT_ATTRIBUTE_LISTENERS) {
